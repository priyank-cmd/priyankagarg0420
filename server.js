...
app.post('/api/chat/stream', async (req, res) => {
  ...
  let timeoutId
  try {
    // Prevent "pending forever" when the upstream model stream hangs.
    const timeoutMs = Number(process.env.CHAT_STREAM_TIMEOUT_MS || 90000)

    const orchestratePromise = orchestrateStream(messages, {
      requestId,
      getCachedChunks,
      buildCore,
      retrieveChunks,
    ...
    if (result.status === 'error') {
      sendSSE({ type: 'error', message: result.err?.message || 'Stream failed' })
    }
    if (result.status === 'timeout') {
      sendSSE({ type: 'error', message: `Timed out after ${Math.round(timeoutMs / 1000)}s.` })
      streamClosed = true
    }
  } finally {
    streamClosed = true
...
app.get('/api/chat/stream', (req, res) => {
  res.status(405).json({
    error: 'Use POST /api/chat/stream with JSON body { messages: [{role, content}], languageHint? }',
  })
})
