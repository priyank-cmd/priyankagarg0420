...
${(context.fullContext || '').slice(0, 25000)}`
  ...
  // Prefer streaming for the chat/stream endpoint so the frontend receives `text` deltas.
  // Only fall back to non-stream generation if the streaming function isn't available.
  if (typeof callClaudeStream !== 'function') {
    try {
      const parsed = await generateDiscoveryPayload(messages, systemPrompt, { callClaude, claudeModel })
